...
./build.sh [target]         # chroot-install ark-os-pi (+ MAVSDK) and bake the board config
./flash.sh /dev/sdX         # write the finished image to an SD card / eMMC
```

The result is a deterministic, turnkey image: every card flashed from it is identical, boots offline, and comes up with ARK-OS installed *and the carrier's hardware already configured* — the production-line analogue of `ark_jetson_kernel --provision`, but far simpler because there is no kernel to build and no NVIDIA recovery-mode flashing.
...
A *target* is a **carrier board × compute module** pair. It decides the hostname and the `config.txt` overlays baked into the image, so one builder produces turnkey images for several boards. Pick one with `./build.sh <target>` (or set `TARGET=` in `versions.env`); the output is named `ark-os-<target>-<version>.img`.
...
All four targets install the **same** `ark-os-pi` deb — there is one ARK-OS build for the Pi, and the golden images differ only in baked configuration (hostname + `config.txt`). Choosing which services run or which configs get written per carrier is left to ARK-OS runtime logic, to be added later.
...
| Imager: create the `pi` user + password | `provision.sh` (baked account) |
| Edit `config.txt` (UART / camera / fan / GPIO overlays) | `provision.sh`, from the selected `targets/*.target` |
| Set the hostname (`pi6x` → `http://pi6x.local/`) | `provision.sh`, from the target |
| Enable SSH | `provision.sh` (`ENABLE_SSH=1`) |
| Install ARK-OS + enable services | `provision.sh` (ark-os-pi deb + `ark-os-firstboot`) |
...
| `build.sh` | Resolves the target, copies the stock image to `staging/`, grows its root partition, loop-mounts it, bind-mounts `/proc /sys /dev`, runs `provision.sh` in an `arm64` chroot (emulated via qemu on an x86 host), and reports how long the build took. Produces `staging/ark-os-<target>-<ver>.img`. |
| `provision.sh` | The chroot core: bakes the `pi` user, sets the hostname, applies the target's `config.txt` edits, enables SSH, blocks daemon starts with a `policy-rc.d` shim, then `apt-get install`s MAVSDK and the `ark-os-pi` deb and verifies them. |
...
- The `ark-os-pi_<ver>_arm64.deb`. Until PR #68 is released, drop a CI-artifact deb into `downloads/` and set `ARK_OS_VERSION` in `versions.env` to its `0.0.0-<sha8>`.
